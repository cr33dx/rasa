...
def mock_core_training(monkeypatch):
    ...
    monkeypatch.setattr(
        rasa.core,
        rasa.core.train.__name__,
        mocked_train_core,
    )
    return mocked_core_training
...
def mock_nlu_training(monkeypatch):
    ...
    monkeypatch.setattr(
        rasa.nlu,
        rasa.nlu.train.__name__,
        mocked_train_nlu,
    )
    return mocked_nlu_training
...
class TestE2e:
    ...
    def test_nlu_and_core_trained_if_no_nlu_data_but_e2e_stories(
        self,
        monkeypatch: MonkeyPatch,
        default_domain_path,  # TODO: types everywhere
        default_stack_config,
        default_e2e_stories_file,
        tmp_path,
    ):
    ...
    def test_new_nlu_data_does_not_retrain_core_if_there_are_no_e2e_stories(
        ...
        train(
            default_domain_path,
            default_stack_config,
            [new_nlu_file, simple_stories_file],
            output=trained_simple_rasa_model,
        )
...
"""
- [x]  `rasa train` was called, nlu data didn't change but e2e stories did and added e2e examples -> need to retrain nlu model
- [x]  `rasa train` was called, but no nlu data are present. However, your stories contain e2e examples. -> We need to train an nlu model.
- [x]  when nlu config changes, `rasa train` should retrain core as well (if e2e!)
